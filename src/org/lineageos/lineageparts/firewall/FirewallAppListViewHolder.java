...
public class FirewallAppListViewHolder extends RecyclerView.ViewHolder implements
     ...
     */
    private FirewallAppListCheckedChangeListener m_CheckListener;

    /**
     * Listener for a click on the app name or app icon.
     */
    private FirewallAppListItemClickedListener m_ClickedListener;
    ...
    public void bindData(FirewallAppInfo info) {
        ...
        // Check if network policies can be changed on this app.
        boolean enabled = UserHandle.isApp(appInfo.getUid());
        wifiCheckBox.setEnabled(enabled);
        cellCheckBox.setEnabled(enabled);
        vpnCheckBox.setEnabled(enabled);

        // mute the checked change listener before changing the checkboxes.
        FirewallAppListCheckedChangeListener temp = m_CheckListener;
        m_CheckListener = null;
        wifiCheckBox.setChecked(appInfo.isWifiEnabled() || !enabled);
        cellCheckBox.setChecked(appInfo.isCellEnabled() || !enabled);
        vpnCheckBox.setChecked(appInfo.isVpnEnabled() || !enabled);
        m_CheckListener = temp;
    }
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (m_CheckListener != null) {
            if (buttonView == wifiCheckBox) {
                ...
                m_CheckListener.onAppListCheckedChange(appInfo, EnumSet.of(FirewallAppListChangeFlag.WIFI));
            } else if (buttonView == cellCheckBox) {
                appInfo.setCellEnabled(isChecked);
                m_CheckListener.onAppListCheckedChange(appInfo, EnumSet.of(FirewallAppListChangeFlag.CELL));
            } else if (buttonView == vpnCheckBox) {
                appInfo.setVpnEnabled(isChecked);
                m_CheckListener.onAppListCheckedChange(appInfo, EnumSet.of(FirewallAppListChangeFlag.VPN));
            }
        }
    }
    ...
    public void onClick(View v) {
        if (m_ClickedListener != null) {
            m_ClickedListener.onAppListItemClicked(appInfo);
        }
    }
}