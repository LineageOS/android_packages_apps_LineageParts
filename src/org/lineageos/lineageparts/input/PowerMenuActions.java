...
import lineageos.app.GlobalActionsManager;
...
public class PowerMenuActions extends SettingsPreferenceFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mLocalUserConfig = mManager.getUserConfig();
    }
    ...
    public void onStart() {
        ...
        if (mScreenshotPref != null) {
            mScreenshotPref.setChecked(mManager.userConfigContains(GLOBAL_ACTION_KEY_SCREENSHOT));
        }

        if (mAirplanePref != null) {
            mAirplanePref.setChecked(mManager.userConfigContains(GLOBAL_ACTION_KEY_AIRPLANE));
        }
        ...
        if (mUsersPref != null) {
            if (!UserHandle.MU_ENABLED || !UserManager.supportsMultipleUsers()) {
                ...
                mUsersPref.setChecked(mManager.userConfigContains(GLOBAL_ACTION_KEY_USERS) &&
                        enabled);
        ...
        if (mBugReportPref != null) {
            mBugReportPref.setChecked(mManager.userConfigContains(GLOBAL_ACTION_KEY_BUGREPORT));
        }

        if (mEmergencyPref != null) {
            mEmergencyPref.setChecked(mManager.userConfigContains(GLOBAL_ACTION_KEY_EMERGENCY));
        }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if (preference == mScreenshotPref) {
            ...
            mManager.updateUserConfig(value, GLOBAL_ACTION_KEY_SCREENSHOT);

        } else if (preference == mAirplanePref) {
            value = mAirplanePref.isChecked();
            mManager.updateUserConfig(value, GLOBAL_ACTION_KEY_AIRPLANE);

        } else if (preference == mUsersPref) {
            value = mUsersPref.isChecked();
            mManager.updateUserConfig(value, GLOBAL_ACTION_KEY_USERS);
            ...
            mManager.updateUserConfig(value, GLOBAL_ACTION_KEY_LOCKDOWN);
            Settings.Secure.putInt(getContentResolver(),
                    Settings.Secure.LOCKDOWN_IN_POWER_MENU, value ? 1 : 0);

        } else if (preference == mEmergencyPref) {
            value = mEmergencyPref.isChecked();
            mManager.updateUserConfig(value, GLOBAL_ACTION_KEY_EMERGENCY);
