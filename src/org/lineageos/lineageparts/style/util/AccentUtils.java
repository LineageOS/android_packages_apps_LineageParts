...
public final class AccentUtils {
    ...
    private static Accent getDefaultAccent(Context context) {
        return new Accent(context.getString(R.string.style_accent_default_name),
                "", getDefaultColorAccent(context));
    }

    @ColorInt
    private static int getDefaultColorAccent(Context context) {
        TypedValue value = new TypedValue();
        TypedArray array = context.obtainStyledAttributes(value.data,
                new int[] { android.R.attr.colorAccent });

        int color = array.getColor(0, DEFAULT_COLOR);
        array.recycle();
